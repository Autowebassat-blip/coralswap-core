...
fn setup_env() -> (Env, Address, Address, Address, Address, Address, Address) {
    ...
    (env, contract_id, factory, token_a, token_b, lp_token, admin)
}

#[test]
fn happy_path_initializes_all_state() {
    let (env, contract_id, factory, token_a, token_b, lp_token, admin) = setup_env();
    let client = PairClient::new(&env, &contract_id);

    client.initialize(&factory, &token_a, &token_b, &lp_token, &30u32, &admin);
...
fn double_init_returns_already_initialized() {
    ...
    let result = client.try_initialize(&factory, &token_a, &token_b, &lp_token, &30u32, &admin);
    assert_eq!(result, Err(Ok(PairError::AlreadyInitialized)));
}

#[test]
fn identical_tokens_returns_error() {
    let (env, contract_id, factory, token_a, _token_b, lp_token, admin) = setup_env();
    let client = PairClient::new(&env, &contract_id);

    let result = client.try_initialize(&factory, &token_a, &token_a, &lp_token, &30u32, &admin);
