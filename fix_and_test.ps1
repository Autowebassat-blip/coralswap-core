...
Write-Host "`n==> Step 1: Build all WASM contracts first" -ForegroundColor Cyan
cargo build --target wasm32-unknown-unknown --release
if ($LASTEXITCODE -ne 0) { Write-Error "WASM build failed"; exit 1 }

Write-Host "`n==> Step 2: Ensure tests/integration/ exists" -ForegroundColor Cyan
New-Item -ItemType Directory -Force -Path "tests\integration" | Out-Null

Write-Host "`n==> Step 3: Write corrected lifecycle.rs" -ForegroundColor Cyan
@'
#![cfg(test)]
...
// contractimport! paths are resolved relative to the workspace root Cargo.toml
mod wasm {
...
fn test_factory_full_pool_lifecycle() {
    ...
    assert!(lp_supply_after_mint > lp_minted, "supply must include MINIMUM_LIQUIDITY");
    ...
    // Reverse swap: token_1 in, token_0 out
...
'@ | Set-Content "tests\integration\lifecycle.rs"

Write-Host "`n==> Step 4: Run integration tests" -ForegroundColor Cyan
cargo test --test lifecycle -p coralswap-integration-tests -- --nocapture